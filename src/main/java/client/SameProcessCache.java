...
public class SameProcessCache<K,V> implements Cache<K,V> {
     ...
     *
     * @param maxObjects 
     * 		maximum number of objects which can be stored before replacement starts
     * 			
     * */
	public SameProcessCache(long maxObjects) {
		cache = CacheBuilder.newBuilder()
			       .maximumSize(maxObjects)
			       .build(
			           new CacheLoader<K,CacheEntry<V>>() {
	...
	/**
     * look up a CacheEntry in the cache.  The CacheEntry may correspond to expired data.  This
     * method can be used to revalidate cached objects whose expiration times have passed
     *
     * @param key 
     * 		key corresponding to value
     * @return 
	...
	public void putAll(Map<K,V> map, long lifetime) {
		...
		for (Map.Entry<K,V> entry : map.entrySet()) {
			CacheEntry<V> cacheEntry = new CacheEntry<V>(entry.getValue(), lifetime + date.getTime());
